...
| PUT | `/v1/prospect-campaign-design/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/prospect-campaign-design/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-prospect-campaign-design:0.1.0 .
```
...
Exposed through the platform Gateway at path prefix `/sd-prospect-campaign-design` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
